...
-- Cast both sides to ::text in all comparisons to avoid text vs uuid type errors.
-- equipment_inventory.organization_id may be stored as text in ictlab's schema.

CREATE POLICY eq_bookings_policy ON equipment_bookings FOR ALL TO authenticated
  USING (
    is_super_admin()
    OR organization_id::text = my_org_id()::text
    OR equipment_id::text IN (SELECT id::text FROM equipment_inventory WHERE organization_id::text = my_org_id()::text)
...
CREATE POLICY eq_bk_settings_policy ON equipment_booking_settings FOR ALL TO authenticated
  USING    (is_super_admin() OR equipment_id::text IN (SELECT id::text FROM equipment_inventory WHERE organization_id::text = my_org_id()::text))
  WITH CHECK (is_super_admin() OR equipment_id::text IN (SELECT id::text FROM equipment_inventory WHERE organization_id::text = my_org_id()::text));

CREATE POLICY eq_bk_blocks_policy ON equipment_booking_blocks FOR ALL TO authenticated
  USING    (is_super_admin() OR organization_id::text = my_org_id()::text)
  WITH CHECK (is_super_admin() OR organization_id::text = my_org_id()::text);

CREATE POLICY retrain_policy ON retraining_requests FOR ALL TO authenticated
  USING    (is_super_admin() OR organization_id::text = my_org_id()::text OR user_id::text = my_user_id()::text)
  WITH CHECK (is_super_admin() OR organization_id::text = my_org_id()::text OR user_id::text = my_user_id()::text);
