...
const LAB_MANAGER_PROFILE_TABS   = ['info','password','dashboard','notifs','team','danger']
const LAB_USER_PROFILE_TABS = ['info','password','dashboard','notifs','team','danger']
  ...
  async function save() {
    ...
    try {
      if (!session?.userId) {
        ...
        // allowed_modules populated into the one we keep, so a manager's per-labUser icon
      ...
      <form onSubmit={e => e.preventDefault()} autoComplete="on" style={{ background:'var(--surface)', borderRadius:'var(--radius-lg)', padding:28, maxWidth:520, width:'100%', maxHeight:'90vh', overflowY:'auto', border:'1px solid var(--border)' }}>
        ...
        <div className="field">
          ...
          <div style={{ border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'8px 12px' }}>
            {SAFETY_STEPS.map(st => {
              const on = form.requiredSteps.includes(st.number)
              return (
                <div key={st.number} onClick={() => toggleStep(st.number)}
                  style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, cursor:'pointer', userSelect:'none', padding:'4px 0' }}>
                  <input type="checkbox" readOnly checked={on} style={{ width:'auto', flexShrink:0 }} />
                  <span style={{ lineHeight:1.3 }}>{st.label}</span>
                </div>
              )
            })}
          </div>
          {/* Unticking everything is the one case that must not be taken
              literally: it would mean no safety training at all. It saves as
              "all steps" and says so, rather than quietly doing the opposite
              of what a blank box looks like. */}
...
function LabManagerListPanel({ toast, session }) {
  ...
  //
  // A role is a `users` row, not a field: Login already fetches every active
  // row matching the email and offers a picker when there is more than one
  // (see Login.jsx — it also back-fills auth_id on all of them). So a second
  // role is a second row sharing the email, and nothing else has to change.
  //
  // No password is set on the new row. Authentication happens once against
  ...
  async function addMemberRole(u, newRole) {
    ...
    const { error } = await sb.from('users').insert({
      ...
      email,
      phone: u.phone || null,
      // Copy auth_id so the new row is usable on the very next login rather
      // than waiting for Login's back-fill pass.
