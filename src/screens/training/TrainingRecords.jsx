  ...
  for (const m of managers) {
    try {
      ...
      if (prefs?.email_training_submitted === true) {
        ...
        if (toEmail) {
          const htmlBody = buildEmailHtml({ title, body, ctaLabel: 'View Training Records →', ctaUrl: 'https://ictlab.labhive.app/?screen=training', prefsUrl: 'https://ictlab.labhive.app/?screen=profile' })
...
async function sendTrainingApprovedNotif(userId, approverName) {
  ...
  try {
    ...
    if (prefs?.email_training_approved === true) {
      ...
      const htmlBody = buildEmailHtml({ title, body, ctaLabel: 'View Training Records →', ctaUrl: 'https://ictlab.labhive.app/?screen=training', prefsUrl: 'https://ictlab.labhive.app/?screen=profile', orgContact })
      ...
      {(hideChrome || equipSubTab === 'training') && (
        <div>
          ...
          {(() => {
            ...
            // Passed exams not yet approved: deduplicated by equipment (keep latest per equipment)
            ...
            // Also exclude equipment that already has a pending schedule (avoid double-counting)
  ...
  function renderTabContent(mode) {
    ...
    switch (subTab) {
      ...
      case 'safety':
        return mode === 'all'
          ? <SafetyTab asTab />
          : <SafetyTab asTab targetUser={selectedUser} />
