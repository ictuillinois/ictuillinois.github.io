...
function ProjectInfo({ project, users, onSaved, isSolo, readOnly }) {
  ...
  // that list is filtered for display, and an export must be the whole project.
...
// ── New Material Modal ──────────────────────────────────────────
function NewMaterialModal({ material, onClose, onCreated, requireProject = false }) {
...
function MaterialInventoryTab({ session, isSolo, onProjectCreated }) {
  ...
  // export can never include an org the viewer cannot see.
                ...
                return (
                  <div key={m.id} className="manage-card"
                    ...
                    {firstPhoto
                      ...
                      : <div style={{ marginBottom: 8 }}><MaterialIcon type={m.material_type} size={34} /></div>}
        ...
        <Modal onClose={() => setEditingProject(null)}>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setEditingProject(null)} title="Close"
              style={{ position: 'absolute', top: -14, left: -14, width: 28, height: 28, borderRadius: '50%', border: '1px solid #e24b4a', background: '#fff', color: '#e24b4a', fontSize: 14, lineHeight: 1, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
              ✕
            </button>
