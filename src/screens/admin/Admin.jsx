...
function UserModal({ user, orgs, defaultOrgId, isSuperAdmin, defaultRole, onClose, onSaved }) {
  ...
  async function saveIconPrefs(userId) {
    ...
    // allowed_modules a lab manager may have separately granted via LabUserIconManager.
  ...
  async function save() {
    ...
    if (user) {
      ...
      if (roles?.includes('lab_user') || role === 'lab_user') await saveIconPrefs(user.id)
...
// plus LAB_MANAGER_PINNED_MODULES for labManagers (empty in this project today, but kept
// in sync with labhive so a future pinned module cannot be offered as an
// inert choice). Unticking such a module changes nothing, so offering it only
// misleads whoever sets the pool.
...
const APP_GLOBAL_MODULES = ALL_MODULES_META.filter(m => inTeam(m) && !FORCED_MODULES.has(m.key) && m.key !== 'mileage' && m.key !== 'labsafety')
    ...
    <div>
      ...
      {userModal && (
        <UserModal
          ...
          defaultRole={isSuperAdmin ? 'admin' : tab === 'labusers' ? 'lab_user' : 'user'}
